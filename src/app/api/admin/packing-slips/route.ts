...
import { renderPackingSlips } from "@/lib/admin/documents";
...
async function pdfResponse(orderNumbers: string[], actorEmail: string) {
  const { buffer, filename } = await renderPackingSlips(orderNumbers, actorEmail);
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not render packing slips." },
      { status: 400 },
    );
  }
}
