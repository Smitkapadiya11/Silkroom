...
export async function PUT(request: Request) {
  ...
  const [settings] = await getDb().insert(storeSettings).values({ id: "default", ...patch }).onConflictDoUpdate({ target: storeSettings.id, set: { ...patch, updatedAt: new Date() } }).returning();
